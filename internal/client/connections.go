...
func (c *Client) ListConnections(ctx context.Context, opts ConnectionListOptions) ([]Connection, error) {
	...
	var resp []Connection
	if err := c.doJSONWithQuery(ctx, http.MethodGet, fmt.Sprintf("%s/connection", BaseAPIPathV2), query, nil, &resp); err != nil {
		return nil, err
	}
	return resp, nil
}
...
func (c *Client) CreateConnection(ctx context.Context, conn *Connection) (*Connection, error) {
	var resp Connection
	if err := c.doJSON(ctx, http.MethodPost, fmt.Sprintf("%s/connection", BaseAPIPathV2), conn, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *Client) UpdateConnection(ctx context.Context, id string, conn *Connection) (*Connection, error) {
	var resp Connection
	if err := c.doJSON(ctx, http.MethodPut, fmt.Sprintf("%s/connection/%s", BaseAPIPathV2, id), conn, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *Client) DeleteConnection(ctx context.Context, id string) error {
	return c.doJSON(ctx, http.MethodDelete, fmt.Sprintf("%s/connection/%s", BaseAPIPathV2, id), nil, nil)
}
