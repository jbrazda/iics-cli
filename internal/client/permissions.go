...
func (c *Client) GetObjectPermissions(ctx context.Context, objectID string) (*ObjectPermission, error) {
	...
	if err := c.doJSON(ctx, http.MethodGet, fmt.Sprintf("%s/objects/%s/permissions", BaseAPIPathV3, objectID), nil, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *Client) SetObjectPermissions(ctx context.Context, objectID string, perms *ObjectPermission) (*ObjectPermission, error) {
	var resp ObjectPermission
	if err := c.doJSON(ctx, http.MethodPut, fmt.Sprintf("%s/objects/%s/permissions", BaseAPIPathV3, objectID), perms, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *Client) DeleteObjectPermissions(ctx context.Context, objectID string) error {
	return c.doJSON(ctx, http.MethodDelete, fmt.Sprintf("%s/objects/%s/permissions", BaseAPIPathV3, objectID), nil, nil)
}
